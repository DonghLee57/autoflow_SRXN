...
def generate_vsepr_vectors(atoms, idx, neighbor_data=None, num_missing=1, cutoff=2.6):
    ...
    if len(vectors) == 0:
        ...
        return [np.array([0.0, 0.0, 1.0])] * num_missing
...
def passivate_surface_coverage_general(
    atoms,
    h_coverage,
    valence_map,
    ...
    if verbose:
        print(f"  [Passivation] Targeting {n_target} {element} sites on {side} surface.")
    ...
    while success < n_target and available:
        ...
        for i_c, cand in enumerate(available):
            ...
            if score > best_score:
                # Overlap check
                _, all_dists_list = get_distances(
                    h_pos_candidate,
                    current_atoms.positions,
                    cell=current_atoms.cell,
                    pbc=current_atoms.pbc,
                )
        ...
        if best_cand_idx != -1:
            ...
            if cand["parent_sym"] == "Si" and element == "H":
                b_len = 1.48
            if cand["parent_sym"] == "O" and element == "H":
                b_len = 0.96
...
class CavityDetector:
    ...
    def find_void_centers(self, top_clearance=4.0):
        ...
        if len(self.prot_idx) == 0:
            ...
            z_max = (
                np.max(self.slab.positions[self.sub_idx, 2]) if len(self.sub_idx) else np.max(self.slab.positions[:, 2])
            )
...
def create_slab_from_bulk(
    ...
    if termination and not top_termination:
        top_termination = termination
    if termination and not bottom_termination:
        bottom_termination = termination
    ...
    if any_term or termination in ["symmetric", "uniform"]:
        if verbose:
            print(
                f"  [Substrate Factory] Executing Layer-Wise Discovery (Bottom={bottom_termination}, Top={top_termination})..."
            )
        ...
        for i in range(len(plane_data)):
            for j in range(i + 1, len(plane_data)):
                ...
                if bottom_termination and bot_match:
                    score += 2000
                if top_termination and top_match:
                    score += 2000
        ...
        if best_pair:
            ...
            if (bottom_termination or top_termination) and not (actual_bot_match and actual_top_match):
                print(
                    f"  [Substrate Factory] WARNING: Requested termination ({bottom_termination}/{top_termination}) could not be fully satisfied. Using best match."
                )
            ...
            if verbose:
                ...
                print(
                    f"  [Substrate Factory] Terminal Bottom: {p_bot['elements']}, Top: {p_top['elements']}, Thickness: {p_top['z'] - p_bot['z']:.2f} A"
                )
        else:
            if verbose:
                print("  [Substrate Factory] Warning: No valid layers found. Falling back.")
    ...
    if supercell_matrix is not None:
        if verbose:
            print(f"  [Substrate Factory] Applying manual supercell matrix: {supercell_matrix}")
        ...
        if verbose:
            ...
            print(
                f"  [Substrate Factory] Selected expansion: {best_n}x{best_m} (Final area: {best_n * best_m * area_prim:.2f} A^2, Score: {best_score:.2f})"
            )
...
def auto_reconstruct_surface(atoms, side="top", verbose=False, **kwargs):
    ...
    if is_group_iv:
        if verbose:
            print("  [AutoReconstructor] Pattern: Covalent Semiconductor -> Dimerization + Buckling.")
        return reconstruct_si100_2x1_buckled(atoms, side=side, verbose=verbose)

    elif is_ionic:
        if verbose:
            print(f"  [AutoReconstructor] Pattern: Ionic/Oxide (dChi={delta_chi:.1f}) -> Rumpling.")
        ...
        if verbose:
            print("  [AutoReconstructor] Pattern: Metallic -> Layer Relaxation + Noise.")
        new_atoms = atoms.copy()
        pos = new_atoms.positions
        ...
        if verbose:
            print("  [AutoReconstructor] Pattern: Mixed/Unknown -> Random Symmetry Breaking.")
