"""Mode-following structural refinement — shared implementation.

Called by the thin ``run_mode_following.py`` wrapper inside each example
sub-directory (precursor/, slab/, …).  All behaviour is driven by
``config.yaml``; the script itself is identical across directories.

Config keys that control structure handling
-------------------------------------------
``paths.input_structure``
    Path to the input VASP file (relative to the config file location).

``paths.center_in_vacuum``   (default: false)
    true  — re-centre the structure in a 10 Å vacuum box before relaxation.
            Use this for isolated molecules.
    false — keep the cell exactly as read (periodic slab, cluster in box, …).

``analysis.vibrational.phva.enabled``   (default: false)
    true  — Partial Hessian; ``phva.frozen_z_ang`` determines the frozen zone.
            A ``FixAtoms`` constraint is automatically applied to the same
            atoms so they remain fixed during every relaxation step.
...
# ---------------------------------------------------------------------------
# Locate the autoflow_srxn package (two levels above this file: examples/../..)
# ---------------------------------------------------------------------------
...
def _load_structure(config: dict, config_dir: str, logger):
    """Load atoms from ``paths.input_structure`` and apply config-driven
    preprocessing.

    Returns
    -------
    atoms      : ASE Atoms — ready for relaxation (constraints already set)
    frozen_idx : list[int] | None — indices of frozen atoms (for slab), or
                 None if no frozen zone.
    """
    paths = config["paths"]
...
def _run_vibration(atoms, engine, vib_name: str, u: float, cycle: int, logger):
    """Run PHVA/FHVA for *cycle*, auto-save qpoints.yaml, return (parser, stats)."""
...
def run_mode_following(config_path: str = "config.yaml"):
    """Run iterative mode-following relaxation.

    Parameters
    ----------
    config_path : str
        Absolute or relative path to the YAML config file.  The working
        directory is changed to the directory containing the config so that
        all relative paths inside the config resolve correctly.
    """
    ...
    # ── Load structure (config-driven preprocessing) ──────────────────────────
    ...
    for cycle in range(1, max_iter + 1):
        ...
        # MultiModeFollower does atoms.copy() internally — re-apply FixAtoms
        # defensively in case the copy lost the constraint
