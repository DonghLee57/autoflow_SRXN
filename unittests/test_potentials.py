...
class TestPotentials(unittest.TestCase):
    def test_engine_init_emt(self):
        ...
        config = {"engine": {"potential": {"backend": "emt", "device": "cpu"}}}
    ...
    def test_engine_relaxation(self):
        ...
        config = {
            "engine": {
                ...
                "relaxation": {"fmax": 0.05, "steps": 10, "optimizer": "BFGS"},
            }
        }
        engine = SimulationEngine(config)
        atoms = molecule("H2")
    ...
    def test_engine_md(self):
        ...
        config = {
            "engine": {
                "potential": {"backend": "emt"},
                "md": {"temperature_K": 300, "timestep_fs": 1.0, "md_steps": 5},
            }
        }
        engine = SimulationEngine(config)
        atoms = molecule("H2")
...
class TestZBLCalculator(unittest.TestCase):
    ...
    def test_zbl_energy_positive(self):
        ...
        from autoflow_srxn.simulation.potentials import ZBLCalculator
    ...
    def test_engine_zbl_disabled(self):
        ...
        config = {"engine": {"potential": {"backend": "emt", "zbl": {"enabled": False}}}}
        engine = SimulationEngine(config)
        calc = engine.get_calculator()
        self.assertIsInstance(calc, EMT)
